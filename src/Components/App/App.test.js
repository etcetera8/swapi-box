...
describe('App', ()=> {
  ...
  it('the apiCall function should setState of scroll text to an obj', async () => {
    const mock = {'title': "Attack of the Clones", 'episode_id': 2,'opening_crawl': "There is unrest in the Galactic", 'release_date': "02-07-2004",'producer': "Rick McCallum"}

    window.fetch = jest.fn().mockImplementation( () => {
      Promise.resolve({
          status: 200,
          results: {episode: 2, title:"the last jedi", releaseData: "05-2017"}
      })
    })
    const wrapper = shallow(<App />);
  ...
  it("the setPlanetState function should update the planet state to the array of planets", async () => {
    ...
    const expected = [{"category": "planets", "climate": "hot", "favorite": false, "name": "tattooine", "population": "10", "residents": [undefined, undefined], "terrain": "permafrost"}]
    window.fetch = jest.fn().mockImplementation(() => 
      Promise.resolve({
        status: 200,
        json: () => Promise.resolve({
  ...
  it("the setFavoriteState should update the favoriteState when a card is clicked on and switch the key from false to favorite", () => {
    wrapper.setState({people: [{name:'luke', species: 'human', homeworld: 'naboo', population: '10', favorite: false}], })
    const expected = {name:'luke', species: 'human', homeworld: 'naboo', population: '10', favorite: true};
    const inst = wrapper.instance();
  ...
  it("the setFavoriteState should take out the card if ran on it again", () => {
    const personArray = [{name:'luke', species: 'human', homeworld: 'naboo', population: '10', favorite: true}];
  ...
  it("should match the snapshot", () => {
    ...
    const personArray = [{name:'luke', species: 'human', homeworld: 'naboo', population: '10', favorite: true}];
    const scrollObject = {scrollText: "Once upon a time", title: "benjamins buttons", releaseDate:"05-07-87", episode: "v"}
    wrapper.setState({
